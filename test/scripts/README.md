...
Integration test scripts for the EST service. These scripts test the EST protocol implementation against a running EST service with OpenBao or Vault backend.

**✅ Portable:** All scripts work on macOS, Linux, BSD, and Alpine Linux (no GNU-specific dependencies).

## Prerequisites

- **Backend**: OpenBao (`bao`) or HashiCorp Vault (`vault`) CLI installed
- **Tools**: `openssl`, `curl`, `jq`, `base64` (standard versions)
- **Optional**: `estclient` (for estclient-based tests)
- **Running Backend**: OpenBao or Vault server running and accessible (can be started with included scripts)
...
### Option A: Start Local Vault with TLS (For Certificate Authentication)

Certificate authentication requires TLS between EST service and Vault. Use the included script:

```bash
# Start Vault with TLS enabled
./start_vault_with_tls.sh

# This creates:
# - Vault CA certificate
# - Vault server TLS certificate
# - Vault configuration with TLS
# - Initialized and unsealed Vault instance

# Stop Vault when done
./stop_vault.sh
```

### Option B: Use Existing Backend

If you have an existing Vault/OpenBao instance:

```bash
export BACKEND_TYPE=vault          # or openbao
...
After starting Vault or configuring your backend connection:
...
| `BACKEND_TYPE` | `openbao` | Backend type: `openbao` or `vault` |
...
export BACKEND_ADDR=https://vault.example.com:8200
...
Prepares the OpenBao/Vault backend for EST testing:
...
Starts a local Vault instance with TLS enabled:
- Generates Vault CA and server certificates
...
Stops the running Vault instance started by `start_vault_with_tls.sh`.
