...
# Sets up OpenBao or Vault as the backend for EST service
...
if [ "$ENABLE_CERT_AUTH" = "true" ]; then
    ...
    log_info "Generating EST service certificate (for Vault/OpenBao auth)..."
...
if [ "$ENABLE_CERT_AUTH" = "true" ]; then
    ...
    echo "  - est-service-cert.pem: EST service backend auth certificate (for Vault/OpenBao)"
