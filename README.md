...
A production-ready EST (Enrollment over Secure Transport) service implementing RFC 7030, with support for both OpenBao and HashiCorp Vault as PKI backends.
...
- **HashiCorp Vault** - Both OSS and Enterprise editions
- Backend auto-detection and abstraction
- Seamless backend switching

### Flexible Authentication
- **HTTP Basic Auth** - Username/password via Vault/OpenBao userpass or AppRole (role_id/secret_id)
- **Bearer Token** - Direct Vault/OpenBao tokens
...
```bash
# 1. Start Vault with TLS (for cert auth testing)
./test/scripts/start_vault_with_tls.sh
...
Integration tests verify PKI operations and authentication against real Vault instances in Docker.
...
"HashiCorp" and "Vault" are trademarks or registered trademarks of HashiCorp, Inc. in the United States and other countries. "OpenBao" is a trademark of the Linux Foundation. This project is not affiliated with, endorsed by, or sponsored by HashiCorp, Inc. or the Linux Foundation.
...
cd test/scripts
./setup_backend.sh    # Setup OpenBao/Vault
