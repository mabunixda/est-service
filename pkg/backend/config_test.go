...
func TestConfig_DefaultValues(t *testing.T) {
	...
	// (This is handled in newVaultBackend and newOpenBaoBackend)

	if cfg.Address != "https://vault.example.com" {
		t.Errorf("Address = %v, want https://vault.example.com", cfg.Address)
	}
...
func TestConfig_TokenRenewalInterval(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The actual default is applied in newVaultBackend/newOpenBaoBackend
...
func TestConfig_TLSSettings(t *testing.T) {
	...
	t.Run("with namespace (Vault Enterprise)", func(t *testing.T) {
		cfg := &Config{
			Address:   "https://vault.example.com",
			Namespace: "tenant1",
		}
