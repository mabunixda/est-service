...
func TestMain(m *testing.M) {
	...
	// Get OpenBao/OpenBao connection info from environment
	openbaoAddr = os.Getenv("BAO_ADDR")
	if openbaoAddr == "" {
		openbaoAddr = os.Getenv("BAO_ADDR")
	}
	if openbaoAddr == "" {
		testLogger.Warn("BAO_ADDR environment variable not set")
		testLogger.Info("Integration tests will be skipped")
		testLogger.Info("To run integration tests, set BAO_ADDR and BAO_TOKEN environment variables")
		testLogger.Info("Example: export BAO_ADDR=https://localhost:8200 BAO_TOKEN=root")
		// Don't initialize backend, tests will skip individually
		os.Exit(0)
	}

	openbaoToken = os.Getenv("BAO_TOKEN")
	if openbaoToken == "" {
		openbaoToken = os.Getenv("BAO_TOKEN")
	}
	if openbaoToken == "" {
		testLogger.Warn("BAO_TOKEN environment variable not set")
		testLogger.Info("Integration tests will be skipped")
		testLogger.Info("To run integration tests, set BAO_ADDR and BAO_TOKEN environment variables")
		os.Exit(0)
	}

	testLogger.Info("Using existing OpenBao/OpenBao instance",
		"addr", openbaoAddr,
		"token_prefix", openbaoToken[:min(10, len(openbaoToken))]+"...")
	...
	if err := initializeOpenBao(ctx); err != nil {
		testLogger.Error("Failed to initialize OpenBao", "error", err)
		os.Exit(1)
	}
	...
	if backendTypeStr != "" {
		...
		backendType = BackendTypeOpenBao // Auto-detect
	}
...
// initializeOpenBao configures PKI and auth methods in the OpenBao instance
// This function is idempotent - it's safe to run multiple times
func initializeOpenBao(ctx context.Context) error {
	apiConfig := api.DefaultConfig()
	apiConfig.Address = openbaoAddr

	// Handle TLS verification
	if os.Getenv("BAO_SKIP_VERIFY") == "true" {
		// For dev/test environments only
		testLogger.Warn("TLS verification disabled (BAO_SKIP_VERIFY=true)")
	}
	...
	if !caExists {
		...
		_, err = client.Logical().Write(pkiMount+"/config/urls", map[string]interface{}{
			"issuing_certificates":    []string{openbaoAddr + "/v1/" + pkiMount + "/ca"},
			"crl_distribution_points": []string{openbaoAddr + "/v1/" + pkiMount + "/crl"},
		})
		if err != nil {
	...
	testLogger.Info("OpenBao initialization complete")
	return nil
}
...
func TestIntegration_Health(t *testing.T) {
	...
	if !health.Initialized {
		t.Error("Expected OpenBao to be initialized")
	}

	if health.Sealed {
		t.Error("Expected OpenBao to be unsealed")
	}
	...
	t.Logf("OpenBao version: %s", health.Version)
}
...
func TestIntegration_ValidateToken(t *testing.T) {
	...
	valid, err := testBackend.ValidateToken(ctx, openbaoToken)
	if err != nil {
...
func TestIntegration_LookupToken(t *testing.T) {
	...
	// Lookup root token
	data, err := testBackend.LookupToken(ctx, openbaoToken)
	if err != nil {
...
// generateUniqueTestName creates a unique name for test resources
// Uses test name + timestamp to avoid conflicts between test runs
func generateUniqueTestName(t *testing.T, prefix string) string {
	t.Helper()
	// Remove special characters from test name
	testName := strings.ReplaceAll(t.Name(), "/", "_")
	testName = strings.ReplaceAll(testName, " ", "_")
	return fmt.Sprintf("%s-%s-%d", prefix, testName, time.Now().UnixNano()%1000000)
}

// cleanupEntity removes an entity by name (best effort, no error if not found)
func cleanupEntity(ctx context.Context, t *testing.T, entityName string) {
	t.Helper()

	// Get API client to look up entity by name
	apiConfig := api.DefaultConfig()
	apiConfig.Address = openbaoAddr
	client, err := api.NewClient(apiConfig)
	if err != nil {
	...
	// List entities and find by name
	resp, err := client.Logical().List("identity/entity/name")
	if err != nil {
		// Entity might not exist, that's fine
		return
	}
	...
	for _, key := range keys {
		if name, ok := key.(string); ok && name == entityName {
			// Delete the entity by name
			_, err := client.Logical().Delete("identity/entity/name/" + name)
			if err != nil {
				t.Logf("Warning: Failed to cleanup entity %s: %v", entityName, err)
			} else {
				t.Logf("Cleaned up entity: %s", entityName)
			}
			return
		}
	}
...
// cleanupAppRole removes an AppRole role (best effort)
func cleanupAppRole(ctx context.Context, t *testing.T, mount, roleName string) {
	t.Helper()
	...
	_, err = client.Logical().Delete(fmt.Sprintf("auth/%s/role/%s", mount, roleName))
	if err == nil {
		t.Logf("Cleaned up AppRole role: %s/%s", mount, roleName)
	}
}

// cleanupUser removes a userpass user (best effort)
func cleanupUser(ctx context.Context, t *testing.T, mount, username string) {
	t.Helper()
	...
	_, err = client.Logical().Delete(fmt.Sprintf("auth/%s/users/%s", mount, username))
	if err == nil {
		t.Logf("Cleaned up user: %s/%s", mount, username)
	}
}
