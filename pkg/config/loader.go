...
func applyDefaults(cfg *Config) error {
	...
	// Support BAO_ADDR or OPENBAO_ADDR (highest priority)
	if envAddr := getEnvWithFallback("BAO_ADDR", "OPENBAO_ADDR"); envAddr != "" {
		cfg.Backend.Address = envAddr
	}
	// Support BAO_TOKEN or OPENBAO_TOKEN (highest priority)
	if envToken := getEnvWithFallback("BAO_TOKEN", "OPENBAO_TOKEN"); envToken != "" {
...
func validate(cfg *Config) error {
	...
	if cfg.Backend.Type != "" {
		cfg.Backend.Type = strings.ToLower(strings.TrimSpace(cfg.Backend.Type))
		if cfg.Backend.Type != "openbao" {
			return fmt.Errorf("backend.type must be 'openbao' when specified")
		}
	}
	...
	if cfg.EST.Authenticators.Cert.Enabled {
		...
		// Validate token TTL format (should be parseable as an OpenBao duration)
		if cfg.EST.Authenticators.Cert.TokenTTL == "" {
		...
		if _, err := time.ParseDuration(cfg.EST.Authenticators.Cert.TokenTTL); err != nil {
			// If it fails, check if it's a valid OpenBao duration format (e.g., "24h", "30d")
			// OpenBao accepts: s, m, h, d formats
