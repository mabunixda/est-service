...
func TestAuthenticateToken_LookupFailure(t *testing.T) {
	...
	// Should still succeed even if lookup fails
	if !result.Authenticated {
		t.Error("Expected authentication to succeed even when lookup fails")
	}
	if result.Identity != "unknown" {
		t.Errorf("Expected identity 'unknown' when lookup fails, got '%s'", result.Identity)
	}
}
...
func TestAuthenticateToken_WhitespaceInToken(t *testing.T) {
	mock := &backend.MockBackend{
		ValidateTokenFunc: func(ctx context.Context, token string) (bool, error) {
			// Token should be trimmed and not contain whitespace
			return token == "no-whitespace", nil
		},
	}
