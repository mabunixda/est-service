...
func (m *Manager) authenticateToken(ctx context.Context, r *http.Request) *Result {
	...
	// Lookup token info (validates token and returns metadata in a single call)
	tokenInfo, err := m.backend.LookupToken(ctx, token)
	if err != nil {
		m.logger.Debug("Token lookup failed (invalid or expired token)", "error", err)
		return &Result{
			Authenticated: false,
			Error:         fmt.Errorf("invalid or expired token: %w", err),
		}
	}

	// Extract identity from token info
	identity := "unknown"
	if tokenInfo != nil {
		if displayName, ok := tokenInfo["display_name"].(string); ok && displayName != "" {
			identity = displayName
		} else if entityID, ok := tokenInfo["entity_id"].(string); ok && entityID != "" {
			identity = entityID
		}
	}
