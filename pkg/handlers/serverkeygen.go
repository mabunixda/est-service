...
type ServerKeyGenConfig struct {
	...
	UseOpenBaoTransit    bool   // If true, use OpenBao Transit engine for key generation
...
func NewServerKeyGenHandler(backend backend.Backend, authMgr *auth.Manager, config *ServerKeyGenConfig, logger *slog.Logger, telemetry Telemetry) *ServerKeyGenHandler {
	...
	// Set Transit defaults if using OpenBao Transit
	if config.UseOpenBaoTransit {
...
func (h *ServerKeyGenHandler) generateKeyPair() (crypto.PrivateKey, crypto.PublicKey, error) {
	...
	// Use OpenBao Transit engine if configured
	if h.config.UseOpenBaoTransit {
...
// generateKeyPairViaTransit generates a key pair using OpenBao Transit engine
func (h *ServerKeyGenHandler) generateKeyPairViaTransit(keyType string) (crypto.PrivateKey, crypto.PublicKey, error) {
	ctx := context.Background()

	h.logger.Debug("Generating key pair via OpenBao Transit engine",
		"key_type", keyType,
	...
	h.logger.Info("Successfully generated key pair via OpenBao Transit",
		"key_type", keyType,
		"key_bits", h.config.DefaultKeySize)
...
func (h *ServerKeyGenHandler) encodePrivateKey(privateKey crypto.PrivateKey, clientCert *x509.Certificate) (*EncryptedPrivateKey, error) {
	...
	if clientCert == nil {
		return nil, fmt.Errorf("client certificate required to encrypt private key password")
	}
	if _, ok := clientCert.PublicKey.(*rsa.PublicKey); !ok {
		return nil, fmt.Errorf("client certificate must be RSA to encrypt private key password")
	}
	...
	// Encrypt the PKCS#8 DER data using AES-256-GCM (authenticated encryption)
	encryptedData, err := encryptWithAESGCM(pkcs8DER, password)
	if err != nil {
		return nil, fmt.Errorf("failed to encrypt private key: %w", err)
	}

	// Create PEM block with encrypted data
	// Note: Using custom PEM type since we're using AES-GCM instead of legacy PEM encryption
	encryptedPEM := &pem.Block{
	...
	// Encode the encrypted PEM block for transmission
	...
	if err != nil {
		return nil, fmt.Errorf("failed to encrypt password with client certificate: %w", err)
	}
...
// encryptWithAESGCM encrypts data using AES-256-GCM (authenticated encryption)
// This provides both confidentiality and authenticity, replacing the deprecated
// x509.EncryptPEMBlock which used insecure CBC mode without authentication.
func encryptWithAESGCM(data, key []byte) ([]byte, error) {
	...
	// The ciphertext contains: nonce || encrypted data || authentication tag
	ciphertext := gcm.Seal(nonce, nonce, data, nil)

	return ciphertext, nil
}

// decryptWithAESGCM decrypts data that was encrypted using encryptWithAESGCM
// This is used for testing and by clients that need to decrypt the private key.
