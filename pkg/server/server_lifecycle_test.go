...
// TestShutdown_WithAuthRateLimiter ensures both rate limiters are shut down
func TestShutdown_WithAuthRateLimiter(t *testing.T) {
	srv := &Server{
		httpServer: &http.Server{
			Addr: ":0",
		},
	...
	if err := srv.Shutdown(ctx); err != nil {
		t.Errorf("Shutdown with auth rate limiter failed: %v", err)
	}
}
...
// TestStart_TLSMode tests starting server in HTTPS mode (TLS configured)
func TestStart_TLSMode(t *testing.T) {
	...
	err = srv.Start(ctx)
	if err != nil && err != context.DeadlineExceeded {
		t.Logf("Start TLS mode completed: %v", err)
	}
}
