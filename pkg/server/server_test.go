...
func TestReadyHandler(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedBody:   "{\"backend\":\"connected\",\"status\":\"ready\"}\n",
		},
		{
...
func TestSetupTLS_ClientAuthConfig(t *testing.T) {
	certFile, keyFile, cleanup := createTestCertFiles(t)
	defer cleanup()

	caFile, caCleanup := createTestCAFile(t, 1)
	defer caCleanup()

	tests := []struct {
		name               string
		clientAuthRequired bool
		wantAuthType       tls.ClientAuthType
	}{
		{
			name:               "client auth required",
			clientAuthRequired: true,
			wantAuthType:       tls.RequireAndVerifyClientCert,
		},
		{
			name:               "client auth optional",
			clientAuthRequired: false,
			wantAuthType:       tls.VerifyClientCertIfGiven,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			srv, err := New(&backend.Client{}, &Config{
				ListenAddr: ":8080",
				PKIMount:   "pki",
				AuthConfig: &auth.Config{},
				EnrollmentConfig: &handlers.EnrollmentConfig{
			...
			tlsConfig, err := srv.setupTLS(&TLSConfig{
				CertFile:           certFile,
				KeyFile:            keyFile,
				ClientCAFile:       caFile,
				ClientAuthRequired: tt.clientAuthRequired,
			})
			if err != nil {
			...
			if tlsConfig.ClientAuth != tt.wantAuthType {
				t.Errorf("ClientAuth = %v, want %v", tlsConfig.ClientAuth, tt.wantAuthType)
			}
		})
	}
}

func TestSetupTLS_ClientCAErrors(t *testing.T) {
	certFile, keyFile, cleanup := createTestCertFiles(t)
	defer cleanup()

	invalidCAFile, invalidCleanup := createTempFileWithContents(t, "not-a-cert")
	defer invalidCleanup()

	emptyCAFile, emptyCleanup := createTempFileWithContents(t, "")
	defer emptyCleanup()
	...
	tests := []struct {
	...
	}{
		{
			...
			wantError: "failed to parse client CA certificate",
		},
		{
			name:      "empty CA file",
			caFile:    emptyCAFile,
		...
		},
		{
			...
			wantError: "failed to read client CA file",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = srv.setupTLS(&TLSConfig{
				CertFile:     certFile,
				KeyFile:      keyFile,
				ClientCAFile: tt.caFile,
			})
			if err == nil {
...
func createTestCAFile(t *testing.T, certCount int) (caFile string, cleanup func()) {
	t.Helper()
	...
	for i := 0; i < certCount; i++ {
		...
		template := &x509.Certificate{
			...
			Subject: pkix.Name{
				CommonName: fmt.Sprintf("test-ca-%d", i+1),
			},
		...
		if err := pem.Encode(caF, &pem.Block{Type: "CERTIFICATE", Bytes: derBytes}); err != nil {
			caF.Close()
			os.Remove(caFile)
	...
	return caFile, cleanup
}

func createTempFileWithContents(t *testing.T, contents string) (filePath string, cleanup func()) {
	t.Helper()

	f, err := os.CreateTemp("", "tmp-*.pem")
	if err != nil {
